...
// Print the data&time
echo '<span id="bartime">It is now: ' . date('Y-m-d') . ' [' . date('g:ia') . ']</span>';
...
if ($pref->prefs['showLangMenu'] == 'true') {
	...
	echo '<option value="Espanol">Spanish</option>';
	echo '</select>';
	echo '</form>';
}
...
echo '<div class="theader">' . date('F, Y', mktime(0, 0, 0, $cset['month'], $cset['day'], $cset['year'])) . '</div>';
echo '</div>';
...
// Preferences
echo '<p><a ' . popup('?view=preferences') . '><img src="images/prefs-trans.png" alt="Preferences" /></a></p>';
